...
for sz in all_subzones:
    ...
    subzone_companies = company_by_subzone.loc[company_by_subzone['subzone_name'] == sz]
    ...
    subzone_freq['sz_p'] = subzone_freq['Count']/total_companies
    
    # Merge national level fraction with subzone freq table
    LQ_table = subzone_freq.merge(sector_national_count[['primary_ssic_str','national_p']], how = 'left', on = 'primary_ssic_str' )
    
    # Calculate LQ
    LQ_table['LQ'] = LQ_table['sz_p']/LQ_table['national_p']
    LQ_table['subzone'] = sz
    
    # Attach all LQ to larger dataset
    sg_LQ_data = pd.concat([sg_LQ_data, LQ_table])

sg_LQ_data.to_csv('sg_subzone_LQ.csv')
